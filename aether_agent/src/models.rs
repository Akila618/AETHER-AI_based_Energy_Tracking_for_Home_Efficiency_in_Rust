...
// NOTE: This function's features are stripped down to only use time features
// which are known for the future, making the prediction useful for forecasting.
...
pub async fn train_model(pool: &MySqlPool) -> Result<TrainResult> {
    ...
    // We only need simulated_date and the total_daily_watts for forecasting based on date
    ...
    // Convert date to numeric format (Days since Epoch)
    let n_samples = history_data.len();
    let n_features = 1; // Only using one feature: Days Since Epoch
    ...
    // Check the targets (total daily watts). Are they all zero?
    println!("[DEBUG] First 5 Targets (Watts): {:?}", &targets[0..5]);

    // Add a check: if all targets are zero, the model has nothing to learn.
    let target_sum: f64 = targets.iter().sum();
    if target_sum < 1.0 {
        println!("[DEBUG] TARGET SUM IS ZERO. Check simulator or database insertion logic.");
        // We will return a non-successful result here to prevent training on bad data
        return Err("Target data (Total Daily Watts) is zero.".into());
    }

    // Standardize features (zero mean, unit variance) to improve numerical stability
    ...
    // Debug: predict on training data to inspect fit quality
...
/// Helper function to get the number of days in a target month/year
...
pub async fn run_prediction_for_month(pool: &MySqlPool, year: i32, month: u32) -> Result<String> {
    // 1. Train model (get model and normalization stats)
    let train = train_model(pool).await?;
    let model = train.model;
    let mean = train.mean;
    let std = train.std;

    // 2. Determine the range and features for prediction
    ...
    // 3. Predict all days simultaneously
    ...
    // Debug: print info about prediction range
    ...
    // 4. Aggregate the results (and print per-day values for debugging)
    for i in 0..num_days as usize {
        let raw_daily = predictions[i];
        let daily_watts = raw_daily.max(0.0); // Ensure usage is not negative
        // compute the day number (1-based)
        let day = i + 1;
        println!("[PREDICT] Day {}-{}-{} -> raw={:.6}W clipped={:.2}W", year, month, day, raw_daily, daily_watts);
        total_predicted_watts += daily_watts;
    }

    // if model produced near-zero total (all clipped to zero), fallback to average daily usage
    ...
    // 5. Convert Watts to Cost (Assuming 30-min per snapshot, so 0.5 hours usage)
